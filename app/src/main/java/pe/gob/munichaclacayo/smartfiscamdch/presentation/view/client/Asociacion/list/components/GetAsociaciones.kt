...
import pe.gob.munichaclacayo.smartfiscamdch.presentation.view.admin.asociacion.list.components.AdminCategoryListContent
...
fun GetCategories(navController: NavHostController ,paddingValues: PaddingValues, vm:AdminAsociacionListViewModel = hiltViewModel()) {
    ...
    when(val response = vm.categoriesResponse){
        ...
        is Resource.Sucess -> {
            ClientCategoryListContent(paddingValues,navController, asociaciones = response.data)
        }
        is Resource.Failure -> {
